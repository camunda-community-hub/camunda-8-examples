...
public class ProcessIntegrationTest {
  ...
  @MockBean
  CreditCardService mockedCreditCardService;
  ...
  public void testPappyPath() throws InterruptedException, TimeoutException {
    ...
    ProcessInstanceEvent processInstance = zeebeClient.newCreateInstanceCommand()
        ...
        .variables(Map.of("customerId", "testCustomer", "orderTotal", 190.0, "expiryDate", "10/24"))
        .send().join();
  ...
  public void testNoCreditCardRequired() throws InterruptedException, TimeoutException {
    given(mockedCustomerService.deductCredit(anyString(), anyDouble(), anyDouble()))
        .willReturn(0.0);
    ProcessInstanceEvent processInstance = zeebeClient.newCreateInstanceCommand()
        .bpmnProcessId("paymentProcess").latestVersion().startBeforeElement("Activity_192ju2a")
        .variables(Map.of("customerId", "testCustomer", "orderTotal", 50.0)).send().join();
    waitForProcessInstanceCompleted(processInstance);
  ...
  public void testIncident() throws InterruptedException, TimeoutException {
    ...
    ProcessInstanceEvent processInstance =
        zeebeClient.newCreateInstanceCommand().bpmnProcessId("paymentProcess").latestVersion()
            .variables(Map.of("customerId", "testCustomer", "orderTotal", 190.0)).send().join();
  ...
  public void testInvalidExpiryDate() throws InterruptedException {
    doThrow(new CreditCardExpiredException("expired")).when(mockedCreditCardService)
        .chargeAmount("1234 5678", "123", "05/23", 100.0);

    ProcessInstanceEvent processInstance = zeebeClient.newCreateInstanceCommand()
        .bpmnProcessId("paymentProcess").latestVersion().variables(Map.of("cardNumber", "1234 5678",
            "cvc", "123", "expiryDate", "05/23", "remainingAmount", 100.0))
        .startBeforeElement("Activity_0nppgjk").send().join();

    waitForProcessInstanceHasPassedElement(processInstance, "Event_0u18e53");

    assertThat(processInstance).isActive().isWaitingAtElements("Activity_0tug4lk")
  ...
  public void testCheckPayment() throws InterruptedException, TimeoutException {
    ProcessInstanceEvent processInstance =
        zeebeClient.newCreateInstanceCommand().bpmnProcessId("paymentProcess").latestVersion()
            .startBeforeElement("Activity_0tug4lk").send().join();
  ...
  public void testRetryPayment() throws InterruptedException, TimeoutException {
    ProcessInstanceEvent processInstance = zeebeClient.newCreateInstanceCommand()
        .bpmnProcessId("paymentProcess").latestVersion().startBeforeElement("Activity_0tug4lk")
        .variables(Map.of("remainingAmount", 10.0)).send().join();
  ...
  protected void completeUserTask(Map<String, Object> variables)
    ...
    List<ActivatedJob> userTaskJobs = zeebeClient.newActivateJobsCommand()
        .jobType(USER_TASK_JOB_TYPE).maxJobsToActivate(1).send().join().getJobs();
