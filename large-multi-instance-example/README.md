...
All sub process or tasks are handled on the same node.
...
For this example I choose a campaign that should generate 10000 letters to be send by mail. The Campaign Process will slide the elements in a number of buckets. Each bucket contains up to 2000 elements. The Letter Process will handle only a single letter.
...
If you want to collect the results from the multi-instance call activity, only the last element is in the list, all other elements are null.
...
There is also a test to proof if future Zeebe versions has resolved this issue: 
